...
$(foreach f,$(wildcard $(COMMON_PATH)/configs/audio/$(TARGET_NOKIA_PLATFORM)/*.conf),\
        $(eval PRODUCT_COPY_FILES += $(f):$(TARGET_COPY_OUT_VENDOR)/etc/$(notdir $f)))
$(foreach f,$(wildcard $(COMMON_PATH)/configs/audio/$(TARGET_NOKIA_PLATFORM)/*.xml),\
        $(eval PRODUCT_COPY_FILES += $(f):$(TARGET_COPY_OUT_VENDOR)/etc/$(notdir $f)))
...
ifeq ($(filter DDV,$(shell echo $(TARGET_PRODUCT) | sed 's/^lineage_//')),)
PRODUCT_COPY_FILES += \
