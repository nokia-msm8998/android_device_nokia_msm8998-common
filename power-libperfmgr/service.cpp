...
int main() {
    ...
    if (android::base::GetBoolProperty(kConfigDebugPathProperty.data(), false)) {
        ...
        LOG(WARNING) << "Nokia Power HAL AIDL Service is using debug config from: " << config_path;
    }
    config_path.append(
            android::base::GetProperty(kConfigProperty.data(), kConfigDefaultFileName.data()));

    LOG(INFO) << "Nokia Power HAL AIDL Service with Extension is starting with config: "
              << config_path;
    ...
    LOG(INFO) << "Nokia Power HAL AIDL Service with Extension is started.";
    ...
    // should not reach
    LOG(ERROR) << "Nokia Power HAL AIDL Service with Extension just died.";
    return EXIT_FAILURE;
}
